...
std::string dot_from_re(std::unique_ptr<re::Re> regex, bool verbose) {
  ...
  while (!stack.empty()) {
    ...
    stack.pop_back();
    if (auto c = dynamic_cast<re::Eps*>(top)) {
    ...
    } else if (auto c = dynamic_cast<re::Char*>(top)) {
      std::string label = std::string("Char: ") + c->c;
      if (verbose) {
        label += "\n";
        label += to_string(c, followpos[c->leaf_count]);
      }
      out << top_idx << " [ shape = circle, label = \"" << label << "\"]\n";
    } else if (auto c = dynamic_cast<re::Kleene*>(top)) {
      ...
      out << top_idx << " [ shape = circle, label = \"" << label << "\" ]\n";
      assert(idx_map.find(c->son) == idx_map.end());
      idx_map[c->son] = idx++;
      stack.push_back(c->son);
      out << top_idx << " -> " << idx_map.at(c->son) << "\n";
    } else if (auto c = dynamic_cast<re::Concat*>(top)) {
      ...
      for (auto son: c->sons) {
        ...
        idx_map[son] = idx++;
        stack.push_back(son);
        out << top_idx << " -> " << idx_map.at(son) << "\n";
      }
    } else if (auto c = dynamic_cast<re::Disjunction*>(top)) {
