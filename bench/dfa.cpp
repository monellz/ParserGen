...
static void re2dfa_engine_init(benchmark::State& state, std::string pattern) {
  for (auto _ : state) {
    auto [re, leaf_count] = parsergen::re::ReEngine::produce(pattern);
    auto start = std::chrono::high_resolution_clock::now();
...
static void re2dfa_engine_produce(benchmark::State& state, std::string pattern) {
  for (auto _ : state) {
    auto [re, leaf_count] = parsergen::re::ReEngine::produce(pattern);
    parsergen::dfa::DfaEngine engine(std::move(re), leaf_count);
...
BENCHMARK_CAPTURE(re2dfa_engine_init, integer, R"([1-9][0-9]*)")->UseManualTime();
BENCHMARK_CAPTURE(re2dfa_engine_produce, integer, R"([1-9][0-9]*)")->UseManualTime();

BENCHMARK_CAPTURE(gen_dfa, hex, std::string(R"(0x[0-9a-fA-F]+)"));
BENCHMARK_CAPTURE(re2dfa, hex, R"(0x[0-9a-fA-F]+)")->UseManualTime();
BENCHMARK_CAPTURE(re2dfa_engine_init, hex, R"(0x[0-9a-fA-F]+)")->UseManualTime();
BENCHMARK_CAPTURE(re2dfa_engine_produce, hex, R"(0x[0-9a-fA-F]+)")->UseManualTime();
BENCHMARK_MAIN();