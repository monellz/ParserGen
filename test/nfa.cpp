...
TEST(basic, metachar) {
  auto meta_dfa = Dfa::from_nfa(Nfa::from_sv(R"(\n)"));
  EXPECT_TRUE(meta_dfa.accept("\n"));
  EXPECT_FALSE(meta_dfa.accept("\b"));
  EXPECT_FALSE(meta_dfa.accept("\\"));
  auto plus_dfa = Dfa::from_nfa(Nfa::from_sv(R"(\+)"));
  EXPECT_TRUE(plus_dfa.accept("+"));
  auto dfa = Dfa::from_nfa(Nfa::from_sv(R"(a+)"));
...
TEST(number, integer) {
  auto single_dfa = Dfa::from_nfa(Nfa::from_sv(R"([0-9])"));
  for (int i = 0; i <= 9; ++i) {
    EXPECT_TRUE(single_dfa.accept(std::to_string(i)));
  }
  EXPECT_FALSE(single_dfa.accept("asdadasd"));

  auto int_dfa = Dfa::from_nfa(Nfa::from_sv(R"([1-9][0-9]*)"));
...
TEST(number, floating) {
  auto float_dfa =
      Dfa::from_nfa(Nfa::from_sv(R"([-+]?[0-9]*[.][0-9]*([eE][-+]?[0-9]+)?)"));
...
TEST(real_case, const_integer) {
  auto dfa = Dfa::from_nfa(Nfa::from_sv(R"(\d+|(0x[0-9a-fA-F]+))"));
  EXPECT_TRUE(dfa.accept("0x123123"));
  EXPECT_TRUE(dfa.accept("123123"));
}

TEST(real_case, ident) {
  auto dfa = Dfa::from_nfa(Nfa::from_sv(R"([_A-Za-z]\w*)"));
